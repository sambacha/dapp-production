...
function unlockedCollateral(userAllowanceESD, userAllowanceUSDC) {
  return (
    userAllowanceESD.comparedTo(MAX_UINT256) === 0 &&
...
function UniswapPool({ user }: { user: string }) {
  ...
  const [pairTotalSupplyUNI, setPairTotalSupplyUNI] = useState(
    new BigNumber(0)
  );
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      ...
      if (!isCancelled) {
        ...
        setPairTotalSupplyUNI(
          toTokenUnitsBN(pairTotalSupplyUNIStr, UNI.decimals)
        );
      }
    }
